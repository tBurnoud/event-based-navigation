...
def convert_to_ros(coord, scale=4.2):
    return np.array([coord[2], -coord[0]-0.05, -coord[1]])*scale
...
if __name__ == "__main__":
    ...
    positions, exec_times = evr.run(clusterType=ClusteringType.CONTOUR)
    ...
    #params list for downscaleFactor
    step = 0.05
    params = np.arange(0.55, 1+step, step)

    #params list for threshold
    # step = 20
    # params = np.arange(0, 250+step, step) 

    #params list for scale
    # step = 0.25
    # params = np.arange(0.0, 5+step, step)
    ...
    for p in params:
        ...
        evr = EventReader(prefix + "exp1.aedat4")
        positions, exec_times = evr.run(downFactor=p, threshold=100, clusterType=ClusteringType.CONTOUR)
        positions.columns = ["time", "x", "y", "z"]
