...
def convert_to_ros(coord):
    return np.array([coord[2], -coord[0], -coord[1]])*4.2
...
if __name__ == "__main__":
    ...
    positions, exec_times = evr.run(sr=35, m_sample=2, clusterType=ClusteringType.FEATURE)
    ...
    for t in np.arange(0,(maxt - mint)  + timeStep, timeStep):
        ...
        #plotting the gathered position
   ...
   #computing average error on position for each axis
    ...
    print("average execution time :", np.mean(exec_times))
